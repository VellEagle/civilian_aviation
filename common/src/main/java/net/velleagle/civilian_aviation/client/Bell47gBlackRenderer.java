...
import net.velleagle.civilian_aviation.entity.Bell47gBlack;

public class Bell47gBlackRenderer extends AircraftEntityRenderer<Bell47gBlack> {
    private static final ResourceLocation ID = CivilianAviation.locate("bell47g_black");
    ...
    private final ModelPartRenderHandler<Bell47gBlack> model = new ModelPartRenderHandler<Bell47gBlack>()
            ...
            .add("propeller_main", (model, object, vcp, entity, ms, light, time, mpr) -> {
                ...
                if (alpha > 0.0f) BBModelRenderer.renderObjectInner(model, object, ms, vcp, light, time, entity,
                        new ModelPartRenderHandler<Bell47gBlack>().vertexConsumerProvider(TRANSLUCENT_PROVIDER),
    ...
    private static float calcBlurAlpha(float power) {
        ...
        if (power >= BLUR_FULL)  return 0.7f;
        return (power - BLUR_START) / (BLUR_FULL - BLUR_START);
    }

    @Override protected ResourceLocation getModelId() { return ID; }
    public Bell47gBlackRenderer(EntityRendererProvider.Context context) { super(context); }
    @Override protected ModelPartRenderHandler<Bell47gBlack> getModel(AircraftEntity entity) { return model; }
}
