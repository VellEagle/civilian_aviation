...
import net.velleagle.civilian_aviation.entity.Bell206Blackstripe;

public class Bell206BlackstripeRenderer extends AircraftEntityRenderer<Bell206Blackstripe> {
    private static final ResourceLocation ID = CivilianAviation.locate("bell206_blackstripe");
    ...
    private final ModelPartRenderHandler<Bell206Blackstripe> model = new ModelPartRenderHandler<Bell206Blackstripe>()
            // 染色パーツ（変更なし）
            .add("dyed_body",
                    ...
                    (model, object, vertexConsumerProvider, entity, matrixStack, light, time, modelPartRenderer) -> {
                        ...
                        if (alpha > 0.0f) {
                            BBModelRenderer.renderObjectInner(
                                    ...
                                    light, time, entity,
                                    new ModelPartRenderHandler<Bell206Blackstripe>()
            ...
            // ── テールローター（デフォルト）propeller_tail ────────────────
            // メインローターと同じタイミングでフェードアウト
            .add("propeller_tail",
                    (model, object, vertexConsumerProvider, entity, matrixStack, light, time, modelPartRenderer) -> {
                        float alpha = calcBladeAlpha(entity.getEnginePower());
                        if (alpha > 0.0f) {
            ...
            // ── テールローター（ブラー）blur_propeller_rear ───────────────
            // メインローターと同じタイミングでフェードイン
            .add("blur_propeller_rear",
                    (model, object, vertexConsumerProvider, entity, matrixStack, light, time, modelPartRenderer) -> {
                        float alpha = calcBlurAlpha(entity.getEnginePower());
                        if (alpha > 0.0f) {
                            BBModelRenderer.renderObjectInner(
    ...
    public Bell206BlackstripeRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    protected ModelPartRenderHandler<Bell206Blackstripe> getModel(AircraftEntity entity) {
        return model;
    }
}