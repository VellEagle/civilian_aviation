...
import net.velleagle.civilian_aviation.entity.SkyhawkRed;

public class SkyhawkRedRenderer extends AircraftEntityRenderer<SkyhawkRed> {
    private static final ResourceLocation ID = CivilianAviation.locate("skyhawk_red");
    ...
    private final ModelPartRenderHandler<SkyhawkRed> model = new ModelPartRenderHandler<SkyhawkRed>()
            // 染色パーツ（変更なし）
            .add("dyed_body",
                    ...
                    (model, object, vertexConsumerProvider, entity, matrixStack, light, time, modelPartRenderer) -> {
                        ...
                        if (alpha > 0.0f) {
                            BBModelRenderer.renderObjectInner(
                                    ...
                                    light, time, entity,
                                    new ModelPartRenderHandler<SkyhawkRed>()
    ...
    public SkyhawkRedRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    protected ModelPartRenderHandler<SkyhawkRed> getModel(AircraftEntity entity) {
        return model;
    }
}