...
import net.velleagle.civilian_aviation.entity.Pzlp11;

public class Pzlp11Renderer extends AircraftEntityRenderer<Pzlp11> {
    private static final ResourceLocation ID = CivilianAviation.locate("pzlp11");
    ...
    private final ModelPartRenderHandler<Pzlp11> model = new ModelPartRenderHandler<Pzlp11>()
            // 染色パーツ（変更なし）
            .add("dyed_body",
                    ...
                    (model, object, vertexConsumerProvider, entity, matrixStack, light, time, modelPartRenderer) -> {
                        ...
                        if (alpha > 0.0f) {
                            BBModelRenderer.renderObjectInner(
                                    ...
                                    light, time, entity,
                                    new ModelPartRenderHandler<Pzlp11>()
    ...
    public Pzlp11Renderer(EntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    protected ModelPartRenderHandler<Pzlp11> getModel(AircraftEntity entity) {
        return model;
    }
}