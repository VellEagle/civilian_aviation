 ...
 * Forge ネイティブの DeferredRegister<SoundEvent> で登録する。
 * immersive_aircraft の Registration.INSTANCE は IA 自身の mod event bus に
 * バインドされているため、civilian_aviation のサウンドを登録すると
 * RegistrySupplier が IA のパイプラインに登録されてしまい、
 * civilian_aviation 側の .get() 呼び出しで "Registry Object not present" が
 * 発生する。
 *
 * アイテム・エンティティ登録は ManOfManyPlanes と同じく RegisterEvent で行う。
 */
...
public class CivilianAviationForge {
    ...
    private static final DeferredRegister<SoundEvent> SOUND_EVENTS =
            DeferredRegister.create(ForgeRegistries.SOUND_EVENTS, CivilianAviation.MOD_ID);
    ...
    private static Supplier<SoundEvent> sound(String name) {
        ...
        RegistryObject<SoundEvent> ro = SOUND_EVENTS.register(name,
